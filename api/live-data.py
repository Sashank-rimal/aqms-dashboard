...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            self.end_headers()
            self.wfile.write(json.dumps({"status": "success", "message": "Data logged"}).encode('utf-8'))
    ...
    def do_GET(self):
        try:
            # Parse URL parameters (e.g., /api/live-data?mode=history)
            parsed_url = urlparse(self.path)
            query_params = parse_qs(parsed_url.query)
            mode = query_params.get('mode', ['live'])[0]

            service = get_sheets_service()
            result = service.values().get(spreadsheetId=SPREADSHEET_ID, range=f"{SHEET_NAME}!A:H").execute()
            ...
            if len(rows) <= 1:
                # Return empty fallback if spreadsheet has no data yet
            ...
            if mode == "history":
                ...
                for r in data_rows:
                    ...
                    history_list.append({
                        ...
                        "mq2": int(float(raw_dict.get("mq2", 0))),
                        "mq135": int(float(raw_dict.get("mq135", 0))),
                        "temperature": int(float(raw_dict.get("temperature", 0))),
                        "humidity": int(float(raw_dict.get("humidity", 0))),
                        "score": int(float(raw_dict.get("score", 0))),
